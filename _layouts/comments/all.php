<div class="tbswp wrap">
    <?php
            ...
            $action = $list->getAction(array('unapprove','approve','trash','spam'));
            $items  = isset($_POST['action'])&&$_POST['action']==-1
            &&isset($_POST['action2'])&&$_POST['action2']==-1?$_POST[$action]:$_POST['action_items'];

            
            if(count($mc_comments->do_actions($items,$action))>0)
            smrtrMsg::info("Comment(s) have been moved to ".$action);
            ...
            where p.post_status="publish" and c.comment_approved not in ("trash","spam")
    ...
    ?>
    <form action="/wp-admin/admin.php?page=comments" method="POST">
        <?php $list->display();?>
    </form>
</div>