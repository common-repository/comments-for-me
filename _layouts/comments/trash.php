<div class="tbswp wrap">
    <?php
            ...
            $action = $list->getAction(array('unapprove','approve','untrash','delete','spam'));
            $items  = $_POST['action']==-1&&$_POST['action2']==-1?$_POST[$action]:$_POST['action_items'];

            if($action=='delete'&&count($mc_comments->do_actions($items,$action))>0){
                smrtrMsg::danger("Comment(s) have been deleted.");
            }
            elseif(count($mc_comments->do_actions($items,$action))>0)
            ...
            $list->setBulkActions(array('untrash'  => 'Untrash'));
            ...
            where p.post_status="publish" and c.comment_approved ="trash"
    ...
    ?>
    <form action="/wp-admin/admin.php?page=comments&view=trash" method="POST">
        <?php $list->display();?>
    </form>
</div>